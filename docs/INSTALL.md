...
`%LOCALAPPDATA%\Programs\NusaShell-Desktop`; Linux uses `curl` or `wget` plus `tar`;
...
macOS installs to `~/Applications/NusaShell-Desktop.app`; the installer removes the quarantine attribute when present. If `~/.local/bin` is not on Linux's PATH, add the exact line printed by the installer to your shell profile.
...
`%LOCALAPPDATA%\\Programs\\NusaShell-Desktop\\`, keeps the active `current` junction
...
installation, remove `~/.local/share/nusashell-desktop`, `~/.local/bin/nusashell-desktop`,
`~/.local/share/applications/nusashell-desktop.desktop`, and optionally
`~/.config/autostart/nusashell-desktop.desktop`. Remove `~/.config/nusashell-desktop` too only
for a full data wipe; it contains durable app state and is not required to
remove the program.

On macOS, remove `~/Applications/NusaShell-Desktop.app` from Finder or the Trash. For a
full data wipe, also remove `~/Library/Application Support/nusashell-desktop/`.

On Windows, remove `%LOCALAPPDATA%\\Programs\\NusaShell-Desktop\\` and the Start Menu
